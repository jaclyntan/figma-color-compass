...
function tintsNshades(r, g, b, steps) {
    ...
    if (steps < 0) {
        //shades
        for (i = 0; i > steps; i--) {
            let diff = ((1 - hsl[2]) / steps) * i, newLightness = Math.abs(hsl[2] - diff), newShade = hsl2rgb(hsl[0], hsl[1], newLightness);
...
figma.ui.onmessage = msg => {
    ...
    if (msg.type === 'update-color') {
        for (const node of figma.currentPage.selection) {
            const color = node['fills'][0]['color'], r = color['r'], g = color['g'], b = color['b'], steps = msg.customSteps;
            var colorObject = {
                shades: {
